...
const ProductStandardGuide = () => {
  ...
  const { viewingStandardGuide, products } = useSelector((state: RootState) => state.productSlice);
  ...
  const handleFileChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if(!viewingStandardGuide) return;
    const file = e.target.files[0];
    const newFile: ProductAttachment = {
      id_anexo_produto: 0,
      arquivo: '',
      id_produto: viewingStandardGuide,
      is_produto_padrao: true, // MARCA COMO PRODUTO PADRÃO
    };
    setLoading(true);
    try {
  ...
  const fetchAttachments = async () => {
    ...
    setLoading(true);
    try {
      const allAttachments = await ProductAttachmentService.getByProduct(
        viewingStandardGuide
      );
      // FILTRA APENAS ANEXOS COM is_produto_padrao = true
      const standardAttachments = allAttachments.filter(
        (att: ProductAttachment) => att.is_produto_padrao === true
      );
      setAttachments(standardAttachments);
    } catch (error: any) {
  ...
  const addAttachment = (file: ProductAttachment) => {
    const product = products.find(
      (product: Product) => product.ID === viewingStandardGuide
    );
    if (!product) return;
    dispatch(
      setProducts(
        products.map((product: Product) =>
          product.ID === viewingStandardGuide
  ...
  const removeAttachment = (id_anexo_produto: number, id_produto: number) => {
    ...
    const updatedAttachments = productToAttachments
      .get(id_produto)
      ?.filter(
      ...
      setProducts(
        products.map((product: Product) =>
          product.ID === id_produto
            ? { ...product, anexos: updatedAttachments }
            : product
        )
      )
    );
  ...
  return (
    <Box>
        ...
        <List sx={{ maxHeight: 180, overflow: "auto" }}>
          {attachments.length === 0 && (
            <Typography variant="body2" color="text.secondary">
              Nenhum guia de produto padrão encontrado.
            </Typography>
          )}
      ...
      <BaseViewFileDialog
        ...
        title={'Visualizar guia de produto padrão'}
      />
      <BaseInputDialog
        open={linkDialogOpen}
        onClose={closeLinkDialog}
        onConfirm={handleAddLink}
        title="Adicionar Link de Guia"
...
export default ProductStandardGuide
