...
const AccessoryAttachmentDialog: React.FC<AccessoryAttachmentDialogProps> = ({
  ...
  const [attachments, setAttachments] = React.useState<AccessoryAttachment[]>([])
  const [loading, setLoading] = React.useState(false)
  const [attachmentBeingDeleted, setAttachmentBeingDeleted] = React.useState<AccessoryAttachment | null>(null)
  const [selectedFile, setSelectedFile] = React.useState<Partial<AccessoryAttachment> | null>(null)
  ...
  const handleUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const payload = {
        id_acessorio_patrimonio: accessory.id_acessorio_patrimonio,
        nome: file.name,
        arquivo: fileUrl,
      }

      const createdFile = await AccessoryAttachmentService.create(payload)
  ...
  const fetchData = React.useCallback(async () => {
    setLoading(true)
    try {
      const attachments = await AccessoryAttachmentService.getMany(accessory.id_acessorio_patrimonio)
  ...
  }, [accessory.id_acessorio_patrimonio, dispatch])

  const handleDelete = React.useCallback(async () => {
    if (attachmentBeingDeleted) {
      try {
        const { id_anexo_acessorio_patrimonio } = attachmentBeingDeleted
        await FirebaseService.delete(attachmentBeingDeleted.arquivo)
        await AccessoryAttachmentService.delete(id_anexo_acessorio_patrimonio)

        setAttachments(
          attachments.filter(
            (att) => att.id_anexo_acessorio_patrimonio !== id_anexo_acessorio_patrimonio
          )
        )
        dispatch(
          setFeedback({
  ...
  const handleAddLink = async () => {
    ...
    setLoading(true)
    try {
      const createdLink = await AccessoryAttachmentService.create(payload)
  ...
  const fileExtensions = ['.pdf', '.jpg', '.jpeg', '.png', '.doc', '.docx', '.xls', '.xlsx']

  const handleAttachmentClick = (attachment: AccessoryAttachment) => {
              ...
              <List sx={{ width: '100%' }}>
                {attachments.map((attachment) => (
                  <ListItem
                    divider
                    sx={{ maxHeight: 60 }}
                    key={attachment.id_anexo_acessorio_patrimonio}
                  >
                    <Stack sx={{ width: '100%' }}>
...
export default AccessoryAttachmentDialog
